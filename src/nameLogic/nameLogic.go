...
func (c Classifier) isNameWithOptions(word string, ignoreEnglish bool) bool {
	/*
	 * Check list so far:
	...
	if len(word) > 2 && string(word[0]) == strings.ToUpper(string(word[0])) && (inside(strings.ToLower(string(word[0])), consonants) || inside(strings.ToLower(string(word[0])), vowels)) {
		...
		for c := 0; c < len(word); c++ {
			...
			if inside(string(word[c]), consonants) {
				// this does ignore vowels, however that's ruled out on way or another with the proceeding algorithm
				foundLetter = true
				i := 1
		...
		// affirm the word isn't a "stopword", and that letters were found in the word
		if inside(word, stopwords) || !foundLetter {
			return false
		}
		// if the option to ignore whether the word is english or not is true,
		// then ignore it.
		if ignoreEnglish {
			return true
		}
		...
		_, isEnglish := c.GetWordType(word)
		...
		if !isEnglish {
			...
			if word[len(word)-1] == 's' {
				...
				_, isEnglish = c.GetWordType(word[:len(word)-1])
				if !isEnglish {
					return true
				}
			} else {
...
// uses the default parameters, and plugs `word` into `Classifier.isNameWithOptions`
func (c Classifier) IsName(word string) bool {
	return c.isNameWithOptions(word, false)
}
...
func (c Classifier) FindNamesInText(text string) map[string]int {
	...
	/*
	 * The following actually has to be an algorithm in itself, since there is a special case
	 * to the name algorithm. This is when a name has a space inbetween 2 words.
	 * For this, we do all the checks except the english check for two words next to eachother.
	 */
	for i := 0; i < len(words); i++ {
		// up until the second last word, check if the next word after it has a capital and is pronounceable
		w := i
		name := ""
		for w < len(words) {
			if c.isNameWithOptions(words[w], true) {
				name += strings.ToLower(words[w])
				w++
			} else {
				break
			}
		...
		if w == i {
			...
			mentions[name] += 1
			// skip over all the consecutive capitalized words
			i += w - i
		}
	}
...
func NewClassifier() (Classifier, error) {
	...
	return Classifier{wordDb: db}, nil
}
